...
import (
	...
	"github.com/sirawann/arigato-shop/pkg/arigatoauth"
...
func (u *usersUsecase) GetPassport(req *users.UserCredentail) (*users.UserPassport, error) {
	...
	// Sign token
	accessToken, err := arigatoauth.NewArigatoAuth(arigatoauth.Access, u.cfg.Jwt(), &users.UserClaims{
		Id:     user.Id,
		RoleId: user.RoleId,
	})
	refreshToken, err := arigatoauth.NewArigatoAuth(arigatoauth.Refresh, u.cfg.Jwt(), &users.UserClaims{
		Id:     user.Id,
		RoleId: user.RoleId,
	})
...
func (u *usersUsecase) RefreshPassport(req *users.UserRefreshCredential) (*users.UserPassport, error) {
	// Parse token
	claims, err := arigatoauth.ParseToken(u.cfg.Jwt(), req.RefreshToken)
	if err != nil {
		return nil, err
	}
	...
	accessToken, err := arigatoauth.NewArigatoAuth(
		arigatoauth.Access,
		u.cfg.Jwt(),
		newClaims,
	)
	...
	refreshToken := arigatoauth.RepeatToken(
		u.cfg.Jwt(),
		newClaims,
